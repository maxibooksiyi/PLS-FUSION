...
bool lineProjectionFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    // std::cout<<"line residual: "<<residual(0)<<" "<<residual(1)<<std::endl;
...
//    sqrt_info.setIdentity();
    ...
    if (jacobians)
    {
        ...
        jaco_l_Lc.setZero();
        jaco_l_Lc.block(0,0,3,3) = Eigen::Matrix3d::Identity();//提取块大小为(p,q),起始于(i,j)	matrix.block(i,j,p,q)
        // std::cout <<jaco_l_Lc<<"\n\n";
        Eigen::Matrix<double, 2, 6> jaco_e_Lc;
        jaco_e_Lc = jaco_e_l * jaco_l_Lc;//pl-vio 公式36的前两项乘积
        //std::cout <<jaco_e_Lc<<"\n\n";
        //std::cout << "jacobian_calculator:" << std::endl;
        if (jacobians[0])//对位姿求导
        {
