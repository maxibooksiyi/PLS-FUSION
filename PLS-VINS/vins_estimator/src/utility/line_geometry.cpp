...
Vector4d pi_from_ppp(Vector3d x1, Vector3d x2, Vector3d x3) {
    Vector4d pi;
    //( x1 - x3 ).cross( x2 - x3 ) 该平面的法向量
    pi << ( x1 - x3 ).cross( x2 - x3 ), - x3.dot( (x1-x3).cross( x2-x3 ) ) ; // d = - x3.dot( (x1-x3).cross( x2-x3 ) )

    return pi;
}

// 两平面相交得到直线的plucker 坐标，前三维为法向量，后三维为方向向量
...
//  空间坐标系下空间线Lw变换到相机坐标系下Lc
Vector6d plk_to_pose( Vector6d plk_w, Eigen::Matrix3d Rcw, Eigen::Vector3d tcw ) {
    Vector3d nw = plk_w.head(3); //该平面的法向量
    Vector3d vw = plk_w.tail(3); //该直线的方向向量
