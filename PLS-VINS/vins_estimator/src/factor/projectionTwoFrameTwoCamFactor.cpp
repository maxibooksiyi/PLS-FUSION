...
bool ProjectionTwoFrameTwoCamFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    Eigen::Vector3d pts_camera_i = pts_i_td / inv_dep_i;//i时刻左目相机坐标系上的点
    Eigen::Vector3d pts_imu_i = qic * pts_camera_i + tic;//i时刻imu坐标系上的点
    Eigen::Vector3d pts_w = Qi * pts_imu_i + Pi;//i时刻世界坐标系上的点
    Eigen::Vector3d pts_imu_j = Qj.inverse() * (pts_w - Pj);//转到右目所在坐标系
    Eigen::Vector3d pts_camera_j = qic2.inverse() * (pts_imu_j - tic2);//转到右目相机坐标系上
