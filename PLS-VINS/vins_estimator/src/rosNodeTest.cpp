...
{
    ROS_INFO("feature_callback");
    map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> featureFrame;
    map<int, vector<pair<int, Eigen::Matrix<double, 4, 1>>>> line_featureFrame;
    ...
    estimator.inputFeature(t, featureFrame,line_featureFrame);
    return;
}
...
{
    ros::init(argc, argv, "pls_vins_estimator"); // ros的初始化ros::init(argc,argv,"my_node_name");// node_name，初始化一个节点
    ...
    //            "~/catkin_ws/src/PLS-VINS/config/euroc/euroc_stereo_imu_config.yaml \n");
    //     return 1;
    // }
