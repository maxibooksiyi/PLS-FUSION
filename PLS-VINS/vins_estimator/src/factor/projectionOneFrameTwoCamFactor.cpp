...
bool ProjectionOneFrameTwoCamFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    Eigen::Vector3d pts_camera_i = pts_i_td / inv_dep_i;//左目 i时刻相机坐标系上的点
    Eigen::Vector3d pts_imu_i = qic * pts_camera_i + tic;//i时刻在imu坐标系下的表示
    Eigen::Vector3d pts_imu_j = pts_imu_i;//同一时刻
    Eigen::Vector3d pts_camera_j = qic2.inverse() * (pts_imu_j - tic2);//右目i时刻在相机坐标系下的表示
    ...
    if (jacobians)
    {
        ...
        reduce << 1. / dep_j, 0, -pts_camera_j(0) / (dep_j * dep_j),
            0, 1. / dep_j, -pts_camera_j(1) / (dep_j * dep_j);    //对应公式10 https://www.jianshu.com/p/72dfd71e7175
#endif
        ...
        if (jacobians[0])
        {
            ...
            jaco_ex.leftCols<3>() = ric2.transpose(); //对位移求导
            jaco_ex.rightCols<3>() = ric2.transpose() * ric * -Utility::skewSymmetric(pts_camera_i);//对旋转求导
