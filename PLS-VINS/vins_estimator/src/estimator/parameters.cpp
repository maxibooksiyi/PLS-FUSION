...
void readParameters(std::string config_file)
{
    ...
    Eigen::Matrix4d T_left,T_right;//左目和右目到IMU的变换矩阵
    ...
    else 
    {
        ...
        cv::cv2eigen(cv_T, T_left);//讲opencv中的Mat格式转化为eigen中的matrix格式
        RIC.push_back(T_left.block<3, 3>(0, 0));//从（0.0）位置开始取一个3*3的矩阵
        TIC.push_back(T_left.block<3, 1>(0, 3));//从（0.3）位置开始取一个3*1的向量
    } 
    
    NUM_OF_CAM = fsSettings["num_of_cam"];
    NUM_OF_LINE= fsSettings["num_of_line"];
    ...
    if(NUM_OF_CAM == 2)
    {
        ...
        cv::cv2eigen(cv_T, T_right);
        RIC.push_back(T_right.block<3, 3>(0, 0));
        TIC.push_back(T_right.block<3, 1>(0, 3));
        
        Eigen::Matrix4d T_C1C2 = Eigen::Matrix4d::Zero();
        T_C1C2 =T_left.inverse()*T_right;
        Eigen::Vector3d t_c1c2;
        t_c1c2=T_C1C2.block<3, 1>(0, 3);
        cout<<t_c1c2.norm()<<endl;
        BASELINE=t_c1c2.norm();
    }
