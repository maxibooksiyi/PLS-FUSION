...
if [ $KITTI -eq 0 ]; then
    CONFIG_IN_DOCKER="/root/catkin_ws/src/PLS-VINS/$(relativePath $(absPath ..) $(absPath ${*: -1}))"
else
    CONFIG_IN_DOCKER="/root/catkin_ws/src/PLS-VINS/$(relativePath $(absPath ..) $(absPath ${*: -2:1}))"
...
if [ $KITTI -eq 0 ]; then
    if [ $LOOP_FUSION -eq 0 ]; then
        ...
        --rm \
        --net=host \
        -v ${VINS_FUSION_DIR}:/root/catkin_ws/src/PLS-VINS/ \
        ros:PLS-VINS \
        /bin/bash -c \
        "cd /root/catkin_ws/; \
        catkin config \
