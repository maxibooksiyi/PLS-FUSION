...
void FeatureTracker::trackImage(map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>>* featureFrame, drawData* data, double _cur_time, const cv::Mat _img, const cv::Mat _img1)
{
    ...
    cur_pts.clear();//左目的像素坐标清空

    // --------------如果上一帧有特征点，就直接进行LK追踪
    if (prev_pts.size() > 0) //上一帧的左目像素坐标
    {
        TicToc t_o;
        vector<uchar> status;
        vector<float> err;
        ...
        if(hasPrediction)
        {
            ...
            cv::calcOpticalFlowPyrLK(prev_img, cur_img, prev_pts, cur_pts, status, err, cv::Size(21, 21), 3);//从左目的前一副图中使用光流来追踪在当前这一帧上面的特征点
    ...
    //cout<<"提取到的点特征数量"<<cur_pts.size()<<endl;
    ...
    if (1)
    {
        ...
        // cout<<"first n_id:"<<n_id<<endl;
        // cout<<n_pts.size()<<endl;
        // n_pts为新检测出来的角点，对于所有新提取出来的特征点，加入到cur_pts
        for (auto &p : n_pts)
        {
            cur_pts.push_back(p);
            ids.push_back(n_id++);//n_id 用来计算新提取出来了多少个角点
            // cout<<"n_id"<<n_id<<endl;
            track_cnt.push_back(1);//初始化特征点的观测次数：1次
        }
        // cout<<"after add:"<<n_id<<endl;
        // printf("feature cnt after add %d\n", (int)ids.size());
    }
    
    cur_un_pts = undistortedPts(cur_pts, m_camera[0]); //得到畸变前的点，转为归一化平面坐标
        ...
        // 把左目的点在右目上找到，然后计算右目上的像素速度。
    {
        ...
        if(!cur_pts.empty())
        {
            ...
            cv::calcOpticalFlowPyrLK(cur_img, rightImg, cur_pts, cur_right_pts, status, err, cv::Size(21, 21), 3);//cur_right_pts,为从左目在右目中找到的匹配点
            ...
            //使用光流在右目中找到左目的特征点，当然他们的特征id是一样的
            ids_right = ids;
    ...
    if(SHOW_TRACK)
    {
        ...
        data->img1=rightImg;
        data->ids=ids;
        data->ids_right=ids_right;
        data->cur_pts=cur_pts;
        data->cur_right_pts=cur_right_pts;
        data->track_cnt=track_cnt;
    }
    ...
    {
        int feature_id = ids[i];//该帧上的特征点编号
        double x, y ,z; //归一化平面上的坐标
        x = cur_un_pts[i].x;
        y = cur_un_pts[i].y;
        z = 1;
        double p_u, p_v; //像素坐标
        ...
        double velocity_x, velocity_y; //像素速度
    ...
    // cout<<"featureFrame:"<<featureFrame.size()<<endl;
    // for(auto a:featureFrame)
    // {
    //     cout<<"点特征id:"<<a.first<<" 被:"<<a.second.size()<<"个相机观测到"<<endl;
    // }
...
{   
    // cout<<"rejectWithF before"<<endl;
    if (cur_pts.size() >= 8)
    {
        // cout<<"rejectWithF if"<<endl;
...
void FeatureTracker::readIntrinsicParameter(const vector<string> &calib_file)
{
    for (size_t i = 0; i < calib_file.size(); i++)
    {
        ...
        cv:Mat tmp_K_,tmp_undist_map1_,tmp_undist_map2_;
        tmp_K_=camera->initUndistortRectifyMap(tmp_undist_map1_,tmp_undist_map2_);
        K_.push_back(tmp_K_);
        undist_map1_.push_back(tmp_undist_map1_);
        undist_map2_.push_back(tmp_undist_map2_);
    }
                               ...
                               map<int, cv::Point2f> &prevLeftPtsMap)
{
    ...
    for (size_t j = 0; j < curLeftPts.size(); j++)
    {
        ...
        cv::circle(imTrack, curLeftPts[j], 2, cv::Scalar(255 * (1 - len), 0, 255 * len), 2);//点越蓝说明被越多帧看到，越红说明被很少的帧一起看到
    }
    ...
    if (!imRight.empty() && stereo_cam)
    {
        for (size_t i = 0; i < curRightPts.size(); i++)
        {
            ...
            cv::circle(imTrack, rightPt, 2, cv::Scalar(0, 255, 0), 2);//点是绿色的
