...
bool ProjectionTwoFrameOneCamFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    Eigen::Vector3d pts_camera_i = pts_i_td / inv_dep_i;//i时刻相机坐标系上的点
    Eigen::Vector3d pts_imu_i = qic * pts_camera_i + tic;//i时刻在imu坐标系下的表示
    Eigen::Vector3d pts_w = Qi * pts_imu_i + Pi; //转到世界坐标系 第i帧的位姿
    Eigen::Vector3d pts_imu_j = Qj.inverse() * (pts_w - Pj);//转到第j帧imu坐标系下
    Eigen::Vector3d pts_camera_j = qic.inverse() * (pts_imu_j - tic);   //转到j时刻相机坐标系上 pts_i在第j帧的重投影
    ...
    // std::cout<<"point residual: "<<residual(0)<<" "<<residual(1)<<std::endl;
#endif
    ...
    if (jacobians)
    {
        ...
        if (jacobians[0])
        {
            ...
            jacobian_pose_i.leftCols<6>() = reduce * jaco_i;//reduce为式10 网址：https://www.jianshu.com/p/72dfd71e7175
...
void ProjectionTwoFrameOneCamFactor::check(double **parameters)
{
    ...
    for (int k = 0; k < 20; k++)
    {
        ...
        Eigen::Vector3d pts_imu_j = Qj.inverse() * (pts_w - Pj);//转到j图像所在坐标系
        Eigen::Vector3d pts_camera_j = qic.inverse() * (pts_imu_j - tic);//转到j时刻相机坐标系上
